...
## تحلیل سبک فرمان‌دهی و نقش آن در موفقیت پروژه

یکی از نکات برجسته این پروژه این است که مسیر آن فقط با «درخواست ساخت فیچر» جلو
نرفت؛ فرمان‌ها به‌تدریج معماری را شکل دادند. کاربر از همان ابتدا پروژه را با یک
نگاه محصولی و اجرایی هدایت کرد: اول مستندات و GitHub، بعد MVP کوچک، بعد تست محلی،
بعد انتشار واقعی، بعد مدیریت خطاها، بعد Providerها، و در نهایت آماده‌سازی برای
production. این ترتیب، بسیار حرفه‌ای بود؛ چون پروژه را از افتادن در دام طراحی
بزرگ و دیررس نجات داد و سریع به خروجی قابل اجرا رساند.

نقطه قوت مهم دیگر، تأکید مداوم روی حداقل نیاز واقعی بود. درخواست‌هایی مثل «دیتابیس
لازم نداریم»، «لاک دیتابیسی مهم نیست»، «این نسخه اولیه است»، و «کد واقعی اما با
حداقل نیازها» باعث شد MVP سبک بماند. این تصمیم‌ها از جنس تصمیم‌های یک مدیر فنی
خوب هستند: اول ارزش واقعی را بساز، بعد پیچیدگی را فقط وقتی اضافه کن که مسئله‌اش
واقعاً ظاهر شده باشد.

در ادامه، فرمان‌ها به‌جای اینکه فقط ظاهر پروژه را کامل کنند، نقاط شکست آینده را
زود آشکار کردند. درخواست اجرای دستی، fixture، اسکریپت‌های لوکال، جدا کردن secretها،
draft شدن وردپرس، و ادامه workflow بدون تصویر، همگی نشان می‌دهند که کاربر پروژه
را فقط برای «کار کردن در حالت ایده‌آل» نمی‌خواست؛ بلکه دنبال سیستمی بود که در
شرایط واقعی، با خطاهای API، محدودیت اکانت‌ها، مشکل وردپرس، نبود تصویر و تفاوت
مدل‌های هوش مصنوعی هم دوام بیاورد. همین نگاه، پروژه را از یک نمونه نمایشی به یک
MVP قابل اتکا نزدیک کرد.

از نظر معماری هم فرمان‌ها بسیار اثرگذار بودند. اصرار روی حذف وابستگی مستقیم به
ChatGPT، ساخت Interface برای LLM، اضافه کردن Groq و Fixture، و بعد جدا کردن Image
Providerها باعث شد سیستم از یک پیاده‌سازی وابسته به یک سرویس، به یک معماری
قابل تعویض و قابل توسعه تبدیل شود. این دقیقاً همان جایی است که کیفیت فرمان‌دهی
کاربر خودش را نشان می‌دهد: مسئله فقط «یک سرویس دیگر اضافه کن» نبود؛ مسئله این بود
که پروژه طوری تغییر کند که در آینده Providerهای جدید، مسیرهای تست و حالت‌های شکست
را راحت‌تر بپذیرد.

در مجموع، مسیر فرمان‌ها نشان می‌دهد کاربر فقط ایده‌دهنده نبود؛ نقش یک مالک محصول
فنی را داشت که هم خروجی می‌خواهد، هم محدودیت واقعی محیط را می‌فهمد، هم وقتی خطا
می‌بیند آن را به بهبود معماری تبدیل می‌کند. اگر بخواهیم این پروژه را در یک ارائه
روایت کنیم، یکی از جذاب‌ترین پیام‌هایش همین است: معماری خوب همیشه از اول کامل
طراحی نمی‌شود؛ گاهی از فرمان‌های دقیق، مرحله‌ای، واقع‌بینانه و پیگیرانه ساخته
می‌شود. در این پروژه، همین جنس فرمان‌ها بود که باعث شد سیستم به‌جای یک اتصال ساده
به AI، به یک زنجیره تولید و انتشار محتوای قابل تنظیم، قابل تست و قابل استقرار
تبدیل شود.
...
متاسفانه اینها دقبقا عین پرامپ های داده شده نیستند و تحلیل هوش مصنوعی از پرامپ ها هستند
...
60. نگه داشتن نسخه قبلی پرامپت‌های OpenAI و Groq با نام `old` و ساخت پرامپت‌های جدید برای تمرکز روی یک خبر ترند یا دو خبر کاملاً مرتبط.
...
### چند خبر نامرتبط، مقاله را مصنوعی می‌کند

وقتی مدل مجبور می‌شود چند خبر از حوزه‌های مختلف را در یک مقاله جمع کند، گاهی بین
آن‌ها ارتباط تحلیلی مصنوعی می‌سازد. درس: prompt باید اجازه بدهد فقط یک خبر مهم و
ترند انتخاب شود، و فقط وقتی دو خبر انتخاب شوند که ارتباط واقعی و مستقیم دارند.
هم‌زمان باید موضوعات سیاسی، امنیتی و حساس به‌صورت صریح از دامنه مقاله حذف شوند.
...
### شکست تصویر نباید کل زنجیره محتوا را متوقف کند

تصویر برای کیفیت خروجی مهم است، اما ستون اصلی workflow مقاله است. بنابراین اگر Provider
تصویر خطا بدهد، تصویری پیدا نشود یا آپلود تصویر در وردپرس شکست بخورد، سیستم با warning
ادامه می‌دهد: وردپرس مقاله را بدون تصویر ذخیره می‌کند، تلگرام متن و لینک را می‌فرستد
و اینستاگرام به‌دلیل نیاز به تصویر skip می‌شود.
...
- ادامه workflow بدون تصویر در صورت خطای image provider یا upload
...
- در صورت شکست تولید/یافتن/آپلود تصویر، مقاله را بدون تصویر ادامه دهد.
