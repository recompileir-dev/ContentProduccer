...
public sealed class OpenApiRouterImageProvider : IImageProvider
{
    ...
    private readonly ILogger<OpenApiRouterImageProvider> _logger;
    private readonly OpenApiRouterOptions _options;

    public OpenApiRouterImageProvider(
        ...
        ILogger<OpenApiRouterImageProvider> logger)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        _logger.LogInformation(
            "Generating article image with OpenApi Router model {Model}. Prompt characters: {PromptCharacters}.",
            _options.ImageModel,
            prompt.Length);
    ...
    private static string ExtractBase64Image(JsonElement root)
    {
            ...
            data[0].TryGetProperty("b64_json", out JsonElement base64))
        {
            return base64.GetString()
                ?? throw new InvalidOperationException("OpenApi Router Image API returned empty image data.");
        }

        throw new InvalidOperationException("OpenApi Router Image API returned no image data.");
    }
}
